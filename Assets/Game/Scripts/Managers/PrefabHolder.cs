...
public class PrefabHolder : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        var wave = config.EnemyWaves[_currentWave];
        if (Time.time - _timeStep > wave.Delay)
        {
            if (_waveEnemiesCount > wave.Count)
            {
                ...
                if (_currentWave > config.EnemyWaves.Length - 1)
                    _currentWave = 0;
    ...
    {
        return InstantiateEntity(config.PlayerProjectile, position);
    }

    public IGameEntity InstantiateEnemyBullet(Vector3 position)
    {
        return InstantiateEntity(config.EnemyProjectile, position);
    }

    public IGameEntity InstantiateEntity(string entityId, Vector2 position)
    {
        var gameplayElement = config.GameElements.First(t => t.Id.Equals(entityId));
    ...
    {
        Instantiate(config.PlayerShip, Vector3.zero, Quaternion.identity);
    }
